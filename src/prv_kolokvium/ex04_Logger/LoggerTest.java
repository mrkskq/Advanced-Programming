package prv_kolokvium.ex04_Logger;
...
class LogSystem<T extends ILog> implements LogProcessor<T> {
    ...
    public LogSystem(ArrayList<T> realLogs) {
        this.logsList = realLogs;
    }
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = new LogProcessor<T>() {
            ...
            public ArrayList<T> processLogs(ArrayList<T> logs) {
                return logs.stream()
                        .filter(l -> l.getType().contains("INFO"))
