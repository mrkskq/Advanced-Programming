package prv_kolokvium.ex17_MojDDV2;
...
class MojDDV{
    ...
    private List<Double> totalPricesWithTaxes;

    public MojDDV(){
        receipts = new LinkedList<Receipt>();
        totalPricesWithTaxes = new ArrayList<Double>();
    }
    ...
    public void printTaxReturns(PrintStream out) {
        ...
        for (Receipt receipt : receipts) {
            ...
            System.out.printf("%10s\t%10s\t%10.5f\n", receipt.getId(), receipt.getTotalPrice(), totalPriceWithTax);
            totalPricesWithTaxes.add(totalPriceWithTax);
        }
    }
    ...
    public void printStatistics(OutputStream outputStream) throws IOException {
        ...
        double min = totalPricesWithTaxes.stream().mapToDouble(p -> p).min().orElse(0.0);
        double max = totalPricesWithTaxes.stream().mapToDouble(p -> p).max().orElse(0.0);
        double sum = totalPricesWithTaxes.stream().mapToDouble(p -> p).sum();
        double count = receipts.size();
        double average = totalPricesWithTaxes.stream().mapToDouble(p -> p).average().orElse(0.0);

        System.out.printf(String.format("min:\t%.3f\n", min));
        System.out.printf(String.format("max:\t%.3f\n", max));
        System.out.printf(String.format("sum:\t%.3f\n", sum));
        System.out.printf(String.format("count:\t%.0f\n", count));
        System.out.printf(String.format("avg:\t%.3f\n", average));
