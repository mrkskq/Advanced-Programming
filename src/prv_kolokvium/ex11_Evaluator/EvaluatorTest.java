package prv_kolokvium.ex11_Evaluator;

import java.util.Arrays;
import java.util.Scanner;
import java.util.stream.Collectors;

interface IEvaluator<Т>{
    boolean evaluate (Т a, Т b);
}

class EvaluatorBuilder {
    //генерички статички метод -> T posle static
...
class Evaluator{
    ...
    //генерички статички метод -> T posle static
    public static <T> boolean evaluateExpression (T left, T right, String operator){
        IEvaluator<T> evaluator = (IEvaluator<T>) EvaluatorBuilder.build(operator);
